...
export default function TeamPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if(teamData){
        const management = teamData.filter(
          (team) => team.department === "Management"
        );
        const team = teamData.filter((team) => team.department === "Team");
        const field = teamData.filter((team) => team.department === "Field");
        setEmployees({
  ...
  const [activeCategory, setActiveCategory] =
    useState<TeamCategory>("management");
          ...
          >
            {currentTeam.map((member, index) => (
              ...
              >
                ...
                <div
                  className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${teamData1[activeCategory][index].gradient}`}
                ></div>
                ...
                <CardContent className="p-8 text-center">
                  ...
                  <div className="relative mx-auto mb-6 w-24 h-24">
                    <div
                      className={`absolute inset-0 rounded-full bg-gradient-to-r ${teamData1[activeCategory][index].gradient} p-1`}
                    >
                      <div className="w-full h-full rounded-full overflow-hidden bg-white dark:bg-gray-900">
                        <Image
                          src={
                            member.profilePicture ||
                            teamData1[activeCategory][index].avatar
                          }
                          alt={member.name}
                          width={96}
                          height={96}
                    ...
                    {/* Icon overlay */}
                    <div
                      className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-gradient-to-r ${teamData1[activeCategory][index].gradient} flex items-center justify-center`}
                    >
                      <span className="text-white text-xs">
                        {teamData1[activeCategory][index].icon}
                      </span>
                    </div>
                  </div>
                  ...
                  <Badge
                    variant="secondary"
                    className={`mb-4 bg-gradient-to-r ${teamData1[activeCategory][index].gradient} text-white`}
                  >
                    {member.designation}
                  </Badge>
